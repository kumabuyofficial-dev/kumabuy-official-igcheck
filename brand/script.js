  ...
  boxes.forEach((box) => {
    box.addEventListener("change", () => {
      ...
      if (checked.length > max) {
        box.checked = false;
        window.alert(`這題最多選 ${max} 個，先保留最重要的方向就好。`);
      }
    });
  });
});
...
const countUp = (element) => {
  ...
  element.textContent = target.toLocaleString("zh-Hant");
  ...
  const tick = (now) => {
    ...
    element.textContent = Math.round(target * eased).toLocaleString("zh-Hant");
